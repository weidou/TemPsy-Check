...
class RandomTraceGenerator(object):
  ...
  def open_file(self):
    # trace output file
    self.outputFile = os.path.join(os.getcwd(), '%s_%d.xmi' % (self.property_id, self.length))
    # trace events locations recording file
    self.locationsFile = os.path.join(os.getcwd(), '%s_%d.locations' % (self.property_id,self.length))
    try:
...
if __name__ == '__main__':
  ...
  various_lengths = [100000, 200000, 300000, 400000, 500000, 600000, 700000, 800000, 900000, 1000000]
  ...
  if length not in various_lengths:
    generator = RandomTraceGenerator(args.id, length, scope, pattern)
    generator.generate_trace()
  else:
    for l in various_lengths:
      if l <= length:
        generator = RandomTraceGenerator(args.id, l, scope, pattern)
        generator.generate_trace()
