...
class RandomTraceGenerator(object):
  ...
  def open_file(self):
    # trace output file
    self.outputFile = os.path.join(os.getcwd(), '%s_%d_%d_between_mult_fixed_number.csv' % (self.property_id, self.length, RandomTraceGenerator.segment_length/1000-1))
    # trace events locations recording file
    self.locationsFile = os.path.join(os.getcwd(), '%s_%d_%d_between_mult_fixed_number.locations.txt' % (self.property_id,self.length,RandomTraceGenerator.segment_length/1000-1))
    try:
    ...
    try:
      ...
      if self.scope.ordinal1 == -1 and self.scope.ordinal2 == -1:
        if length_bound >= self.length:
          ...
          return
        segments = range(1, self.length, self.length/RandomTraceGenerator.fixed_number)
        segments.append(self.length+1)
        for i in range(0, len(segments)-1):
          scope_start = segments[i]
          scope_end = scope_start+RandomTraceGenerator.segment_length-1
          self.write_biscope_segment(scope_start, scope_end, pattern_length_bound)
          if scope_end < segments[i+1]-1:
            self.write_universality(scope_end+1, segments[i+1]-1, RandomTraceGenerator.mask)
      else:
        scope_start = 1
        scope_end = self.length
        self.write_biscope_segment(scope_start, scope_end, pattern_length_bound)
...
if __name__ == '__main__':
  ...
  various_lengths = [2000, 3000, 4000, 5000]
  ...
  for i in range(0,len(various_lengths)):
    RandomTraceGenerator.segment_length = various_lengths[i]
